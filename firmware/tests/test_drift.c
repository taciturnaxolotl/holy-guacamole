...
static void test_peak_differential(void) {
    drift_throttles_t t = drift_compute(0.5f + PI / 2.0f, 0.5f, 0.3f, 0.5f, DRIFT_MOD_SINUSOIDAL);
...
static void test_quadrature_no_diff(void) {
    drift_throttles_t t = drift_compute(0.5f, 0.5f, 0.3f, 0.5f, DRIFT_MOD_SINUSOIDAL);
...
static void test_clamp(void) {
    ...
    drift_throttles_t t = drift_compute(PI / 2.0f, 0.9f, 0.5f, 0.0f, DRIFT_MOD_SINUSOIDAL);
...
static void test_zero_authority(void) {
    for (float th = 0.0f; th < 2.0f * PI; th += 0.5f) {
        drift_throttles_t t = drift_compute(th, 0.4f, 0.0f, 1.2f, DRIFT_MOD_SINUSOIDAL);
