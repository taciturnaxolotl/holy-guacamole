...
void imu_samples_to_meas(const imu_sample_t samples[IMU_COUNT], mat_t *z) {
    ...
    mat_set(z, MEAS_A_RADIAL, 0,
            samples[IMU_A].accel_x * LSM6_ACCEL_G_PER_LSB * G_TO_MS2_L);
    mat_set(z, MEAS_A_TANGENTIAL, 0,
            samples[IMU_A].accel_y * LSM6_ACCEL_G_PER_LSB * G_TO_MS2_L);
    mat_set(z, MEAS_A_GYRO, 0,
            samples[IMU_A].gyro_z * LSM6_GYRO_DPS_PER_LSB * DEG_TO_RAD);

    /* IMU-B: H3LIS331DL. */
    mat_set(z, MEAS_B_RADIAL, 0,
            samples[IMU_B].accel_x * H3LIS_G_PER_LSB * G_TO_MS2_L);
    mat_set(z, MEAS_B_TANGENTIAL, 0,
            samples[IMU_B].accel_y * H3LIS_G_PER_LSB * G_TO_MS2_L);

    /* IMU-C: H3LIS331DL. */
    mat_set(z, MEAS_C_RADIAL, 0,
            samples[IMU_C].accel_x * H3LIS_G_PER_LSB * G_TO_MS2_L);
    mat_set(z, MEAS_C_TANGENTIAL, 0,
            samples[IMU_C].accel_y * H3LIS_G_PER_LSB * G_TO_MS2_L);
}
